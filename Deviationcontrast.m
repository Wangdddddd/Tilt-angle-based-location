...
for j = 1:K
    ...
    for n = 1:N
        k=n*2*pi/N;
        UAVderivative = [-r*sin(k), r*cos(k)];                            %gradient of view point
        UAVposition = [r*cos(k) r*sin(k) h];                             %position of view point
        target_position = [xt yt 0];                                     %target position
        unit_x = [1 0];
        pointing_vector_projection = [xt-r*cos(k) yt-r*sin(k)];          %projection of pointing vector
        unit_pointing_vector = pointing_vector_projection / norm(pointing_vector_projection);
        Shift (:,n) = [r*cos(k) r*sin(k) 0]' ;
        ...
        Beta(n) = acos(h/distance_Ut(n));                         % Pitch angle of camera
        Kapa(n) = pi/2. + k;                                   % Yaw angle of UAV  Kapa belonging to [pi/2,5pi/2]
        ...
        LocatedPosition(:,n) = M{1,n}*[0,0,-distance_Ut(n)]' + [r*cos(k), r*sin(k),0]';
        %     P = [0 0 500];
        %     Q = [1500 0 0];
        %     Slope = norm(P-Q)
        %     oz = 500*ones(1, 360);
...
for k = 1:K
    ...
    shift3(k) = sqrt( (meansum(k,1) - target_position(1))^2 + (meansum(k,2) - target_position(2))^2 );
end
 figure(1)
